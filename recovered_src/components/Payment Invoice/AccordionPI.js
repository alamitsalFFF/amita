...
import AccordionPIHD from "./AccordionPIHD";
import AccordionPIDT from "./AccordionPIDT";
import FloatingActionBar from "../DataFilters/FloatingActionBar";

export default function AccordionPI() {
  ...
  useEffect(() => {
    const fetchCategoryOptions = async () => {
      try {
        const categoryApiUrl = `${API_BASE}/DocConfig/GetDocConfig?category=${category}`;
  ...
  useEffect(() => {
    const fetchWebAddress = async () => {
      try {
        const WebAddressAPI = `${API_BASE}/Module/GetModuleMenu?MenuID=${category}`;
