...
import { DOfromSO } from "../Sales Order/DOfromSO";
import { SIfromSO } from "../Sales Order/SIFromSO";
import { RVfromSO } from "../Sales Order/RVFromSO";
...
import { DOfromSOandStock } from "../Sales Order/DOfromSO_StockFromDO";
import DOManagementComponent from "../Sales Order/DOManagementComponent";
import { DOfromSI } from "./DOfromSI";
import DocStatusSI from "./DocStatusSI";
...
export default function SIAccordionHD({
  ...
  const handleSave = async () => {
    try {
      ...
      // ดึง user ที่ login จาก localStorage (หรือแหล่งข้อมูลอื่น)
      const loginUser = localStorage.getItem("userName") || "";
      dataToSendHD.issueBy = loginUser;
      if (
      ...
      const setCustomer = async (customerData) => {
        const response = await authFetch(`${API_BASE}/Customer/SetCustomer`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            // Add any other necessary headers
      ...
      if (partyCode !== "DEF") {
        ...
        try {
          ...
          if (
            ...
            setCustomerResponse.includes("Customers Created.")
          ) {
  ...
  const handleUpdate = async () => {
    console.log("Updated action from SIHeader");
    try {
  ...
  const handleRC = async (AccDocNoC) => {
    ...
    try {
      ...
      const { value: formValues } = await Swal.fire({
        ...
        // html:
        //   `<input id="swal-bankacc" class="swal2-input" placeholder="Bank Account">` +
        //   `<input id="swal-bankname" class="swal2-input" placeholder="Bank Name" value="${prefillBankName}">` +
        //   `<input id="swal-rcpno" class="swal2-input" placeholder="Receipt No (เลขใบเสร็จ/ใบกำกับที่ได้มา)">` +
        //   `<h6 style="margin: 10px 0;"></h6>` +
        //   `<select id="swal-transtype" class="swal2-select" style="display:block;margin:0 auto">` +
      ...
      try {
        const resp = await RCfromSI(AccDocNoC, bankacc, bankname, transtype, rcpno, duedate, webAddressRC, navigate);
        console.log("RCfromSI response:", resp);
        // accept several shapes returned by PCfromPI
        const rcNo =
          resp?.data?.RCNo ??
          resp?.data?.RCNoString ??
          resp?.data?.AccDocNoC ??
          resp?.RCNo ??
          resp?.data?.data?.RCNo ??
        ...
        if (!rcNo) {
          console.warn("RC created but RCNo not found in response:", resp);
          Swal.fire({
            icon: "success",
            title: "สร้าง RC สำเร็จ",
            text: "ระบบบันทึกแล้ว แต่ไม่พบหมายเลข RC ในผลตอบกลับ โปรดตรวจสอบรายการ",
            showConfirmButton: false,
            timer: 3000,
          });
          // ถ้าต้องการ ให้ fetch/รีเฟรชข้อมูลที่นี่
        } else {
          console.log("Created RC No:", rcNo);
          Swal.fire({
            icon: "success",
        ...
        console.error("Error creating RC Header:", err);
        Swal.fire({
          icon: "error",
          title: "สร้าง Cheque/Transfer ไม่สำเร็จ",
          text: err.message || "Unknown error from RCfromSI",
        });
      }
    } catch (err) {
      console.error("handleRC error:", err);
  ...
  const handleDO = async (AccDocNO) => {
    ...
    // เรียกฟังก์ชัน DOfromSI พร้อมกับส่งค่า AccDocNo และ refno ไปด้วย
    await DOfromSI(AccDocNO, refno, navigate);
  };

  // หน้า ใบแจ้งหนี้ SI 
  const handleSIToJournal = async (AccDocNo) => {
    await SIToJournal(AccDocNo, navigate);
    navigate(`${URL}ReceiveVoucher`, { //ReceiveVoucher เป็นfig address
      state: {
        AccDocNo,
      },
    });
  ...
  const handleRV = async (AccDocNo) => {
    ...
    await RVfromSI(AccDocNo, DocRefNo, webAddressRV, navigate);
    navigate(`${URL}ReceiveVoucher`, {
      state: {
        AccDocNo,
        DocRefNo,
        webAddressRV
  ...
  const [webAddressRC, setWebAddressRC] = useState("");
  const [webAddressRV, setWebAddressRV] = useState("");
  useEffect(() => {
    const fetchWebAddress = async () => {
      try {
        const WebAddressAPI = `${API_BASE}/Module/GetModuleMenu?MenuID=RC`;
        const response = await authFetch(WebAddressAPI);
        if (response.ok) {
          const data = await response.json();
          if (data && data.length > 0) {
            setWebAddressRC(data[0].webAddress);
          }
        }
        // Fetch RC Web Address
        const WebAddressAPIRV = `${API_BASE}/Module/GetModuleMenu?MenuID=RV`;
        const responseRV = await authFetch(WebAddressAPIRV);
        if (responseRV.ok) {
          const data = await responseRV.json();
          if (data && data.length > 0) {
            setWebAddressRV(data[0].webAddress);
          }
        }
      } catch (error) {
  ...
  const handlePartialDOClick = () => {
    setShowDOManagement(true); // ตั้งค่าให้แสดง DOManagementComponent
  };
  ...
  const buttonActions = [
      ...
      ? [
        {
          ...
          name: "All DO(ทำใบส่งของจากใบแจ้งหนี้และยังไม่ลงStock)",
    ...
    //                 </div>
    //               ),
    //               name: "Partial Delivery Out(ส่งสินค้าบางส่วน)",
    //               onClick: handlePartialDOClick,
    //             },
    //           ]
    //         : []),
    //           ...(docStatus === "0" || docStatus === 1
    ...
    //             icon={faTruckMoving}
    //             size="1x"
    //             style={{ color: "#b44901ff" }}
    //           />
    //         </div>
    //       ),
    //       name: "All DO and Stock (ขายสด)",
    //       onClick: () => handleDOandStock(AccDocNO, navigate),
    //     },
    //   ]
    // : []),
      ...
      ? [
        {
          ...
          name: "Cheque Receive(รับเช็ค)",
          onClick: () => handleRC(AccDocNoC),
        },
        {
          icon: (
            <div style={{ display: "flex", alignItems: "center" }}>
              ...
              <FontAwesomeIcon
                icon={faFileInvoice}
                style={{ color: "#da2703ff" }}
                size="1x"
              />
            </div>
          ),
          name: "SI to Journal",
          onClick: () => handleSIToJournal(AccDocNo),
        },
        {
          icon: (
            <div style={{ display: "flex", alignItems: "center" }}>
  ...
  return (
    <div className="row" style={{ padding: "5%", paddingTop: "1px" }}>
      ...
      <div className="col-md-1">&nbsp;</div>
      <div className="col-md-5">
        <DocStatusSI accDocNo={formData.accDocNo} DocType={formData.accDocType} />
      </div>
      <div>&nbsp;</div>
      <div className="col-md-6">
        <TextField
