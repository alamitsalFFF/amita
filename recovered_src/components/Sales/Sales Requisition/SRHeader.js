...
export default function SRHeader() {
  ...
  useEffect(() => {
    const fetchCategoryOptions = async () => {
      try {
        // const categoryApiUrl = `http://103.225.168.137/apiaccbk2/api/Prototype/DocConfig/GetDocConfig`;
        const categoryApiUrl = `${API_BASE}/DocConfig/GetDocConfig`;
        const response = await axios.get(categoryApiUrl);
        // axios returns the data directly, so no need for response.ok or response.json()
  ...
  const handleSave = async () => {
    try {
      ...
      Swal.fire({
        icon: "success",
        title: `บันทึกข้อมูลสำเร็จ SR:${AccDocNo}`,
  ...
  const handleUpdate = async () => {
    try {
      ...
      // ดึง userName จาก token (localStorage) เพื่ออัปเดต issueBy
      const userName = localStorage.getItem("userName") || "";
      dataToSend.issueBy = userName;
      ...
      //   return;
      // }
      // console.log("userToken:", localStorage.getItem("userToken")); // ตรวจสอบ token ที่เก็บไว้ใน localStorage
      const response = await authFetch(
  ...
  const handleDelete = async () => {
    try {
      ...
      Swal.fire({
        icon: "success",
        title: `ลบข้อมูลสำเร็จ SR:${response.accDocNo}`,
        // text: "ข้อมูลสินค้า/บริการถูกบันทึกเรียบร้อยแล้ว",
        showConfirmButton: false,
        timer: 2000,
      });
    ...
    try {
      ...
      navigate(`/uitestacc/SRListDT?accDocNo=${accDocNo}`, {
        state: {
          ...
          partyName: partyName,
          nameCategory: nameCategory,
          selectedDocConfigID: selectedDocConfigID, // ส่งค่า selectedDocConfigID ไปยังหน้าถัดไป
        },
      });
    } catch (error) {
  ...
  const buttonActions = [
    ...
    //           onApproved={() => {
    //             setFormData({ ...formData, docStatus: 1 });
    //             Swal.fire({
    //               icon: "success",
    //               title: "Approved!",
    //               timer: 1500,
    ...
    <div className="row" style={{ padding: "5%", paddingTop: "1px" }}>
        ...
        <Box sx={customerModalStyle}>
          ...
          <List>
            {getPaginatedData().map((customer) => (
              <ListItem key={customer.customerCode} disablePadding>
                <ListItemButton
      ...
      <div className="col-md-5">
        {/* <TextField
          id="docStatus"
          label="DocStatus"
          value={statusName || formData.docStatus}
          // value={statusName}
          type="text"
          // type="number"
          variant="standard"
        ...
        /> */}
        <DocStatusDisplay accDocNo={formData.accDocNo} />
      </div>
      <div>&nbsp;</div>
      <div className="col-md-6">
        <TextField
