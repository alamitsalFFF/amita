...
import DocStatusColor from "../../DataFilters/Status";
...
function SRList() {
  ...
  const filteredSR = transactionall.filter((transaction) => {
    ...
    if (filterStartDate && filterEndDate) {
      ...
      const transactionDate = dayjs(transaction.AccBatchDate); // วันที่เอกสาร
  ...
  const handleEditClick = (transaction, index) => {
    ...
    // navigate(`${URL}AccordionSR`, { state: { initialAccDocNo: transaction.AccDocNo } });
    navigate(`${URL}AccordionSR?accDocNo=${transaction.AccDocNo}`);
  };

  const handleEditClick1 = (transaction, index) => {
    setSelectedIndex(index); // set index ที่เลือก
    dispatch(setAccDocNo(transaction.AccDocNo));
    dispatch(setPartyName(transaction.PartyName));
    dispatch(setAccDocType(transaction.AccDocType));
  ...
  const handleAddNew = () => {
    ...
    navigate(`${URL}AccordionSR?accDocType=${accDocType}`, {
      state: { isNew: true },
        ...
        <ul>
          {groupedTransactions.length > 0 ? (
            ...
            <p style={{ textAlign: "center", marginTop: "20px" }}>
              ไม่พบรายการ Sales Requisition ในช่วงวันที่ **{filterStartDate ?
