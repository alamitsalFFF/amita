...
export default function SOHeader() {
    ...
    try {
      ...
      const setCustomer = async (customerData) => {
        const response = await authFetch(
          `${API_BASE}/Prototype/Customer/SetCustomer`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDO = async (AccDocNo) => {
    ...
    await DOfromSO(AccDocNo, navigate);
  ...
  const handleSISV = async (AccDocNo) => {
  ...
  const handleSI = async (AccDocNo) => {
    // const DocRefNo = formData.docRefNo;
    // const userId = "admin"; // หรือจะดึงจากระบบ login
    await SIfromSO(AccDocNo, navigate);
  ...
  const buttonActions = [
    ...
    {
      icon: (
        <FontAwesomeIcon icon={faPrint} style={{ color: "blue" }} size="2x" />
      ),
      name: "Print",
  ...
  return (
    <div className="row" style={{ padding: "5%", paddingTop: "1px" }}>
      <h2 style={{ textAlign: "center" }} onClick={handleGoMenu}>Sales Order</h2> &nbsp;
      {/* <div style={{ display: "flex" }}>
        ...
        <div
          ...
          style={{ paddingLeft: "10px", cursor: "pointer", display: "grid" }}
        >
          <FontAwesomeIcon icon={faPrint} size="2x" style={{ color: "blue" }} />
        </div>
        <div
          className="col-1"
          style={{
