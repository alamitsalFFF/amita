...
export default function AccordionRVHD({
  apiData,
  setApiData,
  currentIndex,
  setCurrentIndex,
  ...
  const [doctype, setDoctype] = React.useState("RV"); // ตั้งค่าเริ่มต้นเป็น PV
  ...
  const fetchDataFromApi = async (journalNoParam = null) => {
    ...
    try {
      ...
      if (response.status !== 200) {
        console.error("Error fetching vRV_All:", response.statusText);
        setApiData([]);
        return [];
      }
      ...
      console.error("Error fetching data via vRV_All:", error);
      setApiData([]);
      return [];
    }
  };
  ...
  useEffect(() => {
    ...
    if (newJournal && newJournal !== currentAccDocNo) {
      ...
      console.log("AccordionRVHD setCurrentAccDocNo ->", newJournal);
    }
    ...
    const fetchCustomerOptions = async () => {
      try {
        const response = await axios.get(
          `http://103.225.168.137/apiaccbk2/api/Prototype/Customer/GetCustomer`
        );
  ...
  useEffect(() => {
    ...
    const fetchWithView = async () => {
      ...
      try {
        ...
        console.error("Error fetching HD list via vRV_All:", err);
      } finally {
  ...
  useEffect(() => {
    ...
    if (journalToUse) {
      // ... existing logic to set currentAccDocNo ...
      // But wait, the previous logic was just initializing navInitRef or similar? 
      // Let's see the actual code in the file again to be precise.
      // The previous view showed lines 896-900.
      // It seems I need to see the full block to replace it correctly without breaking context.
      // But based on the previous read (step 56/72), the code was:
      /*
      useEffect(() => {
        const journalFromState = location.state?.journalNo || location.state?.JournalNo;
        const params = new URLSearchParams(location.search);
        const journalFromQuery = params.get("journalNo");
        const journalToUse = journalFromState || journalFromQuery;
      */
    }

    if (journalToUse && journalToUse !== currentAccDocNo && !navInitRef.current) {
      setCurrentAccDocNo(journalToUse);
      if (JournalNo !== journalToUse) {
        dispatch(setAccDocNo(journalToUse)); // Ensure Redux is synced on initial load from URL
      }
      navInitRef.current = true;
    }
  }, [location.state, location.search, currentAccDocNo, dispatch, JournalNo]);
