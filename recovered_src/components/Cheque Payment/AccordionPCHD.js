...
export default function AccordionPCHD({ apiData, setApiData, currentIndex, setCurrentIndex, setCurrentAccDocNo }) {
  ...
  useEffect(() => {
    ...
    const fetchWebAddress = async () => {
      try {
        const WebAddressAPI = `${API_BASE}/Module/GetModuleMenu?MenuID=DI`;
  ...
  const handleSave = async () => {
    try {
      ...
      Swal.fire({
        icon: "success",
        title: `บันทึกข้อมูลสำเร็จ PC:${AccDocNo}`,
  ...
  // หน้า ใบแจ้งค่าใช้จ่าย PC ปุ่ม จ่ายเงิน/จ่ายชำระเงิน
  // const handlePV = async (accDocNo) => {
  //   const docRefNo = formData.docRefNo;
  //   await PVfromPC(accDocNo, docRefNo, navigate);
  //   navigate({
  ...
  const handlePV = async (accDocNo) => {
    ...
    // เรียกฟังก์ชัน PVfromPC พร้อมกับส่งค่า AccDocNo และ acccode ไปด้วย
    await PVfromPC(accDocNo, docRefNo, acccode, navigate);
    navigate({
      state: { accDocNo, docRefNo, acccode },
    });
  };
  ...
  //   // เรียกฟังก์ชัน DIfromPC พร้อมกับส่งค่า accDocNo และ refno ไปด้วย
  //   await DIfromPC(accDocNo, refno, navigate);
  // };
  const handleDI = async (accDocNo) => {
    ...
    // 3. เรียกฟังก์ชัน DOfromRC พร้อมกับส่งค่า accDocNo, refno และ duedate ไปด้วย
    await DIfromPC(accDocNo, refno, navigate, duedate, authFetch, webAddress); // Note: Added dependencies
  };
  // -------------------------------
  ...
  return (
    <div className="row" style={{ padding: "5%", paddingTop: "1px" }}>
      ...
      <div className="col-md-1">&nbsp;</div>
      <div className="col-md-5">
        <DocStatusPC accDocNo={formData.accDocNo} DocType={formData.accDocType} />
      </div>
