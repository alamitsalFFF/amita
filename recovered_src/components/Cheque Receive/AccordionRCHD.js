...
import { CancelPC } from "../Cheque Payment/CancelPC"; //รอสโตร์
import { GetGLTemplate } from "../purchase/Purchase Order/GetGLTemplate";
import { PVfromPC } from "../Cheque Payment/PVFromPC";
import GLTemplateModal from "../purchase/Purchase Order/GLTemplateModal";
import DocStatusPC from "../Cheque Payment/DocStatusPC";
...
export default function AccordionRCHD({ apiData, setApiData, currentIndex, setCurrentIndex, setCurrentAccDocNo }) {
  ...
  useEffect(() => {
    const fetchCategoryOptions = async () => {
      try {
        ...
        console.log('data docConfig:', data);
  ...
  const handleSave = async () => {
    try {
      ...
      Swal.fire({
        icon: "success",
        title: `บันทึกข้อมูลสำเร็จ RC:${AccDocNo}`,
  ...
  const handleCancel = async () => {
    await CancelPC(AccDocNoC, navigate);
  };
  ...
  const handleDO = async (accDocNo) => {
    ...
    const { value: formValues } = await Swal.fire({
      title: "Create DO from RC",
      html:
        '<div style="text-align: left;">' +
        '<label for="swal-input1" style="display: block; margin-bottom: 5px;">Reference No:</label>' +
        `<input id="swal-input1" class="swal2-input" placeholder="(Ref. No.) ${accDocNo}" style="margin-bottom: 15px;">` +
    ...
    // 3. เรียกฟังก์ชัน DOfromRC พร้อมกับส่งค่า accDocNo, refno, duedate, authFetch, และ webAddress ไปด้วย
    await DOfromRC(accDocNo, refno, navigate, duedate, authFetch, webAddressDO);
  };


  const handleRV = async (accDocNo) => {
    const docRefNo = formData.docRefNo;
    // 1. แสดง Modal เพื่อรับค่า acccode จากผู้ใช้
    const { value: acccode } = await Swal.fire({
      title: "กรอก AccCode เช่น 1154-00(ภาษีซื้อ)",
      input: "text",
      inputLabel: `(AccCode)`,
      showCancelButton: true,
      inputValidator: (value) => {
        if (!value) {
          return "กรุณากรอกAccCode!";
        }
      },
    });
    ...
    // 3. ถ้าผู้ใช้กรอกค่า acccode แล้ว
    // เรียกฟังก์ชัน RVfromRC พร้อมกับส่งค่า AccDocNo และ acccode ไปด้วย
    await RVfromRC(accDocNo, docRefNo, acccode, webAddressRV, navigate);
    navigate({
      state: { accDocNo, docRefNo, acccode, webAddressRV },
    });
  };
  ...
  const [isCSV, setIsCSV] = useState(0);
  const [webAddressDO, setWebAddressDO] = useState("");
  const [webAddressRV, setWebAddressRV] = useState("");
  useEffect(() => {
    const fetchWebAddress = async () => {
      try {
        const WebAddressAPI = `${API_BASE}/Module/GetModuleMenu?MenuID=DO`;
        const response = await authFetch(WebAddressAPI);
        if (response.ok) {
          const data = await response.json();
          if (data && data.length > 0) {
            setWebAddressDO(data[0].webAddress);
          }
        }
  ...
  const buttonActions = [
    ...
    },
    {
      ...
      name: "Receive Voucher(ใบสำคัญรับเงิน)",
      onClick: () => handleRV(accDocNo, docRefNo, navigate),
    },
    // ส่วนของ DO
      ...
      ? [
        {
          ...
          name: "All DO(ทำใบส่งของจากใบรับเช็คและยังไม่ลงStock)",
  ...
  return (
    <div className="row" style={{ padding: "5%", paddingTop: "1px" }}>
      ...
      <div className="col-md-1">&nbsp;</div>
      <div className="col-md-5">
        <DocStatusRC accDocNo={formData.accDocNo} DocType={formData.accDocType} />
      </div>
