...
import DocStatusPO from "./DocStatusPO";
...
export default function POHeader() {
      ...
      if (partyCode !== "DEF") {
        ...
        try {
          ...
          if (
            ...
            // Swal.fire({
            //   icon: "error",
            //   title: "เกิดข้อผิดพลาดในการสร้าง Supplier",
  ...
  const handleDI = async (AccDocNo) => {
    ...
    await DIfromPO(AccDocNo, navigate);
  ...
  const handlePIPV = async (AccDocNo) => {
    ...
    await PVfromPO(AccDocNo, DocRefNo, navigate);
    // const DocRefNo = formData.docRefNo;
    console.log("AccDocNo:", AccDocNo);
  ...
  const handlePI = async (AccDocNo) => {
    // const DocRefNo = formData.docRefNo;
    // const userId = "admin"; // หรือจะดึงจากระบบ login
    await PIfromPO(AccDocNo, navigate);
    ...
    <div className="row" style={{ padding: "5%", paddingTop: "1px" }}>
      ...
      <div className="col-md-5">
        {/* <TextField
          ...
          style={{ width: "100%" }}
        /> */}
        <DocStatusPO accDocNo={formData.accDocNo} />
      </div>
